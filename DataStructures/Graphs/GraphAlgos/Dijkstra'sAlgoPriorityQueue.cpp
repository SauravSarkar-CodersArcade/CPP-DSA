...
public:
    vector<int> dijkstra(int V, vector<vector<pair<int, int>>> &adj, int S){
        ...
        // Initialize the distance array to infinite values
        // To suggest that the nodes are not reachable
        vector<int> distTo(V, INT_MAX);
        // Make the distance to source node 0
        distTo[S] = 0;
        pq.push({0,S});
        ...
        while (!pq.empty()){
            ...
            // Traverse to all it's adjacent nodes
            // Check whether the previous distance is greater than the current distance
        ...
        // Return the distance array which has the shortest distance to all the nodes
